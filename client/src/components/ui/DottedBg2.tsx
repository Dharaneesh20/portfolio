...
// Interactive WebGL Shader Background (ogl)
...
void main() {
  ...
  // Interactive ripple effect based on mouse position
  vec2 mouseUv = (uMouse - 0.5) * vec2(aspect, 1.0) + 0.5;
  float distToMouse = length(uv - mouseUv);
  float mouseRipple = sin(distToMouse * 18.0 - uTime * 5.0) * exp(-distToMouse * 3.8) * uMouseActive * 0.3;

  float hue = abs(snoise(vec3((uv + vec2(mouseRipple)) * uFrequency, uTime * uSpeed)));
  hue = fract(hue + mouseRipple * 0.2);
...
const MAX_COLORS = 10;
const DEFAULT_COLORS = ["#1e3a8a", "#2563eb", "#3b82f6", "#4f46e5", "#6366f1", "#8b5cf6", "#a855f7", "#c084fc"];
...
export interface DottedBg2Props {
  frequency?: number;
  speed?: number;
  bgColor?: string;
  colors?: string[];
...
export default function DottedBg2({
  ...
  const targetMouseActiveRef = useRef(0);
  ...
  useEffect(() => {
    ...
    const handlePointerMove = (e: MouseEvent | TouchEvent) => {
      ...
      const normX = (clientX - rect.left) / width;
      const normY = 1.0 - (clientY - rect.top) / height;
    ...
    const update = (time: number) => {
      ...
      mouseActiveRef.current +=
        (targetMouseActiveRef.current - mouseActiveRef.current) * 0.05;

      perlinProgram.uniforms.uTime.value = time * 0.001;
      perlinProgram.uniforms.uMouse.value = [
        currentMouseRef.current.x,
        currentMouseRef.current.y,
      ];
      perlinProgram.uniforms.uMouseActive.value = mouseActiveRef.current;
    ...
    if (effectivePlay) {
      if (rafIdRef.current == null) {
        ...
        rafIdRef.current = requestAnimationFrame(function update(time: number) {
          ...
          perlin.uniforms.uMouse.value = [
            currentMouseRef.current.x,
            currentMouseRef.current.y,
          ];
          perlin.uniforms.uMouseActive.value = mouseActiveRef.current;
